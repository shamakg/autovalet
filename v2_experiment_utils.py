...
# TODO: Make this work
def get_bounding_boxes(parking_scenario, debug = True, life_time = 1):
    """Extract bounding boxes from a parking scenario"""
    moving_cars_bbs = []
    traffic_cone_bbs = []
    walker_bbs = []
    
    world = parking_scenario.world
    ...
    def oriented_bbox(actor, bb):
        """Return an axis-aligned bbox that tightly wraps the actor's oriented bbox."""
        transform = actor.get_transform()
        bb_world = transform.transform(bb.location)
        yaw = np.deg2rad(transform.rotation.yaw)
        cos_y, sin_y = np.cos(yaw), np.sin(yaw)
        dx = abs(bb.extent.x * cos_y) + abs(bb.extent.y * sin_y)
        dy = abs(bb.extent.x * sin_y) + abs(bb.extent.y * cos_y)
        return [bb_world.x - dx, bb_world.y - dy, bb_world.x + dx, bb_world.y + dy]

    for vehicle in vehicles:
        # Skip ego vehicle
        if vehicle.id == parking_scenario.car.actor.id:
            continue

        # Skip parked cars (they're already in parked_cars_bbs)
        if vehicle.id in parked_car_ids:
            continue

        bbox = oriented_bbox(vehicle, vehicle.bounding_box)
        moving_cars_bbs.append(bbox)
        _draw_bb(world, bbox, carla.Color(0, 0, 255), life_time)

    props = all_actors.filter('static.prop.*')
    for prop in props:
        if 'cone' in prop.type_id.lower():
            bbox = oriented_bbox(prop, prop.bounding_box)
            traffic_cone_bbs.append(bbox)
            _draw_bb(world, bbox, carla.Color(0, 0, 255), life_time)

    walkers = all_actors.filter('walker.*')
    for walker in walkers:
        if 'controller' in walker.type_id:
            continue
...
def _draw_bb(world, bbox, color, life_time):
    min_x, min_y, max_x, max_y = bbox[0], bbox[1], bbox[2], bbox[3]
    z = 0.5 
    
    corners = [
        carla.Location(x=min_x, y=min_y, z=z),
        carla.Location(x=max_x, y=min_y, z=z),
        carla.Location(x=max_x, y=max_y, z=z),
        carla.Location(x=min_x, y=max_y, z=z),
    ]
    for i in range(4):
...
def town04_spawn_parked_cars_with_doors(world, spawn_points, skip, num_random_cars):
    blueprints = world.get_blueprint_library().filter('vehicle.*.*')
    blueprints = [bp for bp in blueprints if bp.id in PARKED_VEHICLES]
    blueprints = [
        bp for bp in blueprints
        if bp.has_attribute("has_dynamic_doors") and
           bp.get_attribute("has_dynamic_doors").as_bool()
    ]
    parked_cars = []
    parked_cars_bbs = []
    parked_cars_and_spots_bbs = []
