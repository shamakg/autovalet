...
from v2 import plan_hybrid_a_star, TrajectoryPoint
...
if USE_DIFFUSION:
    from diffusion_planner import diffusion_plan, load_model
...
## just removed check_static_collisions
class ObstacleMap():
...
class CarlaCar():
    ...
    def iou(self):
        ...
        if self.debug:
            for v in car_vertices:
                self.world.debug.draw_string(carla.Location(x=v[0], y=v[1]), 'o', draw_shadow=False, color=carla.Color(r=255, g=0, b=0), life_time=0.1, persistent_lines=True)
            for v in destination_vertices:
                self.world.debug.draw_string(carla.Location(x=v[0], y=v[1]), 'o', draw_shadow=False, color=carla.Color(r=0, g=255, b=0), life_time=0.1, persistent_lines=True)
...
class Car():
    ...
    def _call_planner(self, cur: TrajectoryPoint, destination: TrajectoryPoint, obs: ObstacleMap) -> list[TrajectoryPoint]:
        """
        Try diffusion planner; fall back to Hybrid A* on any failure or if USE_DIFFUSION=False.
        Also validates the diffusion trajectory with the existing collision checker before using it.
        """
        if USE_DIFFUSION:
            try:
                return diffusion_plan(cur, destination, obs)
                # if traj and len(traj) > 2:
                #     if not obs.check_collision(traj):
                #         self._diffusion_cache = traj
                #         return traj
                #     else:
                #         print("[Diffusion] trajectory has collision, falling back to Hybrid A*")
            except Exception as e:
                print(f"[Diffusion] failed: {e}, falling back to Hybrid A*")
    ...
    def plan(self):
        if self.mode == Mode.FAILED:
            return
        new_trajectory = self._call_planner(self.cur, self.destination, self.obs)
        if new_trajectory:
            for i in range(1, TRAJECTORY_EXTENSION+1):
                new_trajectory.append(self.destination.offset(i/3))
            self.ti = 1
