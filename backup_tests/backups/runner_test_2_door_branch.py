...
from srunner.scenarios.vehicle_opens_door_parking import VehicleOpensDoorTwoWaysParking
...
## THis method, at least to my knowledge, is difficult to implement since it spawns a NEW car (doesnt use existing)
def car_open_scenario(world, car, ego_location, parked_car):
    ...
    scenario = VehicleOpensDoorTwoWaysParking(
        world,
        [car.actor],
        parked_car,
        config=config,
...
def run_scenario(world, destination_parking_spot, parked_spots, ious, recording_file):
    ...
    try:
        ...
        pedestrian_spawn_times = [random.randint(0, 75) for i in range(random.randint(1, 1))]
        ...
        if len(open_possible) > 0:
            ...
            print(f"✅ Door scenario created (will open automatically when close)")

        if door_scenario:
            all_scenarios.append(door_scenario)

        scenario_manager = ScenarioManager(timeout=240, debug_mode=False)
        for scenario in all_scenarios:
            scenario_manager.load_scenario(scenario)

        i = 0
        collision_logged = set()
        while not is_done(car):
            ...
            #                 if scenario == door_scenario:
            #                     print(f"\n🚨 DOOR COLLISION at frame {i}!\n")
            #                 else:
            #                     print(f"\n🚨 PEDESTRIAN COLLISION at frame {i}!\n")
