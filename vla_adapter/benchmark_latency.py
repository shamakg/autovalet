### Latency Evaluation version of benchmark.py
import argparse
import json
import os
import signal
...
def run_scenario(
    ...
    try:
        ...
        # ---------------------------------------------------------------
               # Latency Buffer
        control_queue = [] # list of (arrival_time_ms, control)
        current_applied_control = carla.VehicleControl(brake=1.0)
        ...
        while not skip_requested and not adapter.is_done(parking_scenario.car.car.destination):
            ...
            current_time_ms = timestamp.elapsed_seconds * 1000.0

            # Run model every tick (identical to regular benchmark)
            raw_control = adapter.run_step_testbed(timestamp)
            measured_inf_ms = getattr(adapter, 'last_inference_latency_ms', 0.0)
            measured_pipe_ms = getattr(adapter, 'last_pipeline_latency_ms', 0.0)

            # Queue the control to arrive after the delay ("action is delayed by XXX ms")
            effective_latency = measured_inf_ms if dynamic_latency else latency_ms
            control_queue.append((current_time_ms + effective_latency, raw_control))

            # Collect stats for reporting
            all_measured_latencies.append(measured_inf_ms)
            all_pipeline_latencies.append(measured_pipe_ms)

            print(f"t={current_time_ms:.1f}ms, Inf: {measured_inf_ms:.1f}ms, Delay: {effective_latency:.1f}ms")

            # Retrieve control that has "arrived"
            while control_queue and control_queue[0][0] <= current_time_ms:
                current_applied_control = control_queue.pop(0)[1]

            print(f"applying: t={current_applied_control.throttle:.3f} s={current_applied_control.steer:.3f} (Time: {current_time_ms:.1f}ms, Delay: {effective_latency:.1f}ms)")
            parking_scenario.car.actor.apply_control(current_applied_control)
        ...
        weighted_iou = iou
        if mode == HardMode.DoorMode:
            # Safety score: 0 if dist < 0.05m, 1 if dist > 0.30m, linear in between
            safety_score = np.clip((min_door_dist - 0.05) / (0.30 - 0.05), 0.0, 1.0)
            # Weighted score: S = IOU * w1 + Safety * w2 (w1=0.5, w2=0.5)
            weighted_iou = (iou * 0.5) + (safety_score * 0.5)
            print(f'Door Safety Min Dist: {min_door_dist:.3f}m, Safety Score: {safety_score:.3f}')
            print(f'Weighted IOU: {weighted_iou:.3f}')
        ...
        if all_measured_latencies:
            inf_lats = np.array(all_measured_latencies)
            pipe_lats = np.array(all_pipeline_latencies)
            print(f"\n--- LATENCY STATS ---")
            print(f"          INFERENCE      PIPELINE")
            print(f"Mean:     {np.mean(inf_lats):6.2f}ms    {np.mean(pipe_lats):6.2f}ms")
            print(f"Median:   {np.median(inf_lats):6.2f}ms    {np.median(pipe_lats):6.2f}ms")
            print(f"P95:      {np.percentile(inf_lats, 95):6.2f}ms    {np.percentile(pipe_lats, 95):6.2f}ms")
            print(f"P99:      {np.percentile(inf_lats, 99):6.2f}ms    {np.percentile(pipe_lats, 99):6.2f}ms")
            print("-------------------------------------------\n")

        return iou, weighted_iou, collisions_ref[0], min_door_dist, all_measured_latencies, all_pipeline_latencies
...
def main():
    parser = argparse.ArgumentParser(description="Latency Evaluation benchmark.")
    parser.add_argument("--mode", type=lambda x: CollisionMode[x], required=False, default=None, help="Scenario Mode")
    parser.add_argument("--latency", type=int, default=200, help="Fixed latency in ms (if not dynamic)")
    parser.add_argument("--dynamic-latency", action="store_true", help="Use actual model execution time as latency")
    parser.add_argument("--no-comparison", action="store_true", help="Only run with specified latency")
    parser.add_argument("--num-scenarios", type=int, default=None, help="Limit number of scenarios to run")
    args = parser.parse_args()
    
    latencies_to_test = [0, args.latency]
    if args.no_comparison:
        latencies_to_test = [args.latency]
        
    all_summaries = {}
    
    client = load_client()
    
    for lat_idx, lat in enumerate(latencies_to_test):
        is_delayed_run = (lat_idx > 0 or args.no_comparison) and lat > 0
        is_dynamic = is_delayed_run and args.dynamic_latency
        
        # Create a descriptive suffix for the results folder
        mode_str = args.mode.name if args.mode else "DoorMode"
        
        if is_dynamic:
            print(f" EVALUATING LATENCY: DYNAMIC (Mode: {mode_str})")
        else:
            print(f" EVALUATING LATENCY: {lat}ms (Mode: {mode_str})")
        print(f"{'='*20}\n")
        
        suffix = f"lat{lat}ms_{mode_str}"
        run_dir = make_run_dir(suffix=suffix)
        all_recording_paths = []
        car_list = [None]
        ...
        try:
            ...
            actual_collisions = []
            all_inf_latencies = []
            scenarios = SCENARIOS if args.num_scenarios is None else SCENARIOS[:args.num_scenarios]
            for i, (destination_parking_spot, parked_spots) in enumerate(scenarios):
                ...
                result = run_scenario(
                    ...
                    mode=args.mode,
                    latency_ms=lat,
                    dynamic_latency=is_dynamic,
                )
                if result is not None:
                    all_inf_latencies.extend(result[4])  # all_measured_latencies
        ...
        summary = {
            ...
            'inference_latencies_ms': all_inf_latencies,
        }
        all_summaries[lat] = summary
        
        with open(os.path.join(run_dir, 'results.json'), 'w') as f:
            json.dump(summary, f, indent=2)
        print(f'Saved results.json for {lat}ms latency')

    # Final Comparison Report
    print(f"\n\n{'#'*40}")
    print(f" LATENCY COMPARISON REPORT")
    print(f"{'#'*40}")
    print(f"{'Latency':<10} | {'Collisions':<12} | {'Mean IOU':<10} | {'Mean Weighted IOU':<18}")
    print(f"{'-'*60}")
    for lat in latencies_to_test:
        s = all_summaries[lat]
        print(f"{lat:<10} | {s['actual_total_collisions']:<12} | {s['mean_iou']:<10.3f} | {s['mean_weighted_iou']:<18.3f}")
    print(f"{'#'*40}\n")
