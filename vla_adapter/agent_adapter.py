...
class CarlaCar():
    def __init__(self, world, blueprint, spawn_point, destination, destination_bb, debug=False, agent = None):
        self.world = world
        ...
        self.car = Car((destination.x, destination.y), self.gnss_sensor, self.time_sensor, self.collision_sensor, self.world, agent=agent)
...
class Car():
    def __init__(self, destination: Tuple[float, float], gnss_sensor: CarlaGnssSensor, time_sensor: CarlaTimeSensor, collision_sensor: CarlaCollisionSensor, world):
        ...
        ##### BEGINNNING AGENT ADAPTER INTEGRATION ~~~~
        self.agent = agent
    ...
    def run_step(self, parked_car_ids=set()):
        ...
        ### IF we have an agent, let's use it and bypass all of this other logic
        if self.agent is not None:
            return self.agent.run_step(self)
